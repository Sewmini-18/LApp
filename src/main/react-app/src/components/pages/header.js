...
import { Navbar, NavDropdown, Nav, Col, Form, ButtonGroup, Button } from 'react-bootstrap'
...
class Header extends Component {
  ...
  setColor1 = event => {
    ...
    const currentUser = AuthService.getCurrentUser();
    const userId = currentUser.id;
    axios.put("http://localhost:8080/api/auth/color/" + userId, theme)
      .then(response => {
        if (response.data != null) { this.setState({ "show": true }); } else {
