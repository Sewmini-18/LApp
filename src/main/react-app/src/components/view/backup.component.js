    ...
    const data = {
      ...
      rows: [
        ...this.state.folders.map((data, i) => ({
          ...
          date: data.date,
          action3: (
              <input type="checkbox" defaultChecked={data.select}
                     onChange={(e) => this.handleChangeChk(e.target.checked, i)}/>
          ),
          action1: (
              <Link to={{
  ...
  return (
    <div>
      ...
      <div className="container p-3"> {
        this.state.loading ? (
            <div>
              <MDBDataTable hover entriesOptions={[10, 20, 50, 100]} entries={10} data={data} materialSearch striped
                            bordered style={{backgroundColor: "#f0f5fa"}}/>
